...
from effects.double import DebiasedML
from effects.DataManager import DataTypeManager, prep_data
import pandas
...
from effects.meta.xlearner import BaseXClassifier, BaseXRegressor
...
class EffectEstimator:
    ...
    def prepare_data(
        ...
        if isinstance(treatment, ContinuousTreatmentParams):
            if not ignore_outcome:
                data[treatment.column] = data[treatment.column].astype(float)
            treatment_type = "continuous"
        elif isinstance(treatment, CategoricalTreatmentParams):
            if not ignore_outcome:
                data[treatment.column] = data[treatment.column].astype(str)
        ...
        if not ignore_outcome:
            treatment_vector: pandas.Series = treatment.fit_transform(data)
        else:
            treatment_vector = pandas.Series([True] * len(data))

        if not ignore_outcome:
            outcome_vector: pandas.Series = outcome.fit_transform(data)
        else:
            outcome_vector = pandas.Series([0] * len(data))

        X: pandas.DataFrame = data.loc[:, covariates]
    ...
    def predict(self, data: Union[pandas.DataFrame, dict]) -> pandas.DataFrame:
        ...
        if treatment_type == "categorical":
            if self.learner_x is None:
                raise ValueError("Learner is not fitted")
            ite = self.learner_x.predict(X.values)
        elif treatment_type == "continuous":
            if self.learner_double is None:
                raise ValueError("Learner is not fitted")

            raise NotImplementedError("Continuous treatment is not implemented")
            # ite = self.learner_double.predict(X, treatment_series, outcome_series, outcome_type)
        else:
            raise TypeError("outcome type is wrong")
    ...
    def fit_predict(
        ...
        for covariate in covariates:
            ...
            # we measure the base
            if isCategorical:
                grouper = output_data.groupby(covariate, observed=False)
            else:
                n = 5
                good = False
                while n > 2:
                    try:
                        ...
                        grouper = output_data.groupby("quantile", observed=False)
                        good = True
                        break
                    except:
                        n = n - 1
